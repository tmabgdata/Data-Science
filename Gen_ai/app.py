...
def generate_images(prompt, negative_prompt, num_images_per_prompt,
                    num_interference_steps, height, width, seed, guidance_scale):
    ...
    images = pipeline(prompt=prompt, num_images_per_prompt=num_images_per_prompt, 
                        ...
                        height = height, width = width, generator = generator, guidance_scale = guidance_scale)['images']
    return images

with st.sidebar:
    st.header("Configurações da Geração de Imagem")
